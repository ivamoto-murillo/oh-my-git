...
if [ -n "${BASH_VERSION}" ]; then
    ...
    function custom_build_prompt {
        ...
        local white_on_green="${white}${background_red}"
        ...
        if [[ $is_a_git_repo == true ]]; then
            ...
            if [[ $detached == true ]]; then
                prompt+=$(enrich_append $detached $omg_detached_symbol "${white_on_green}")
                ...
                if [[ $has_upstream == false ]]; then
                    ...
                    if [[ $has_diverged == true ]]; then
                        prompt+=$(enrich_append true "-${commits_behind} ${omg_has_diverged_symbol} +${commits_ahead}" "${white_on_green}")
                    else
                        if [[ $commits_behind -gt 0 ]]; then
                            prompt+=$(enrich_append true "-${commits_behind} ${white_on_green}${omg_can_fast_forward_symbol}${black_on_green} --" "${black_on_green}")
                        fi
                        if [[ $commits_ahead -gt 0 ]]; then
                            prompt+=$(enrich_append true "-- ${white_on_green}${omg_should_push_symbol} ${black_on_green} +${commits_ahead}" "${black_on_green}")
                        fi
            ...
            prompt+="${omg_last_symbol_color}${green}${reset}\n"
